...
<div wire:poll.3s.visible
    ...
    {{-- Header (profile now lives in the bottom nav) --}}
    <header class="mb-4 flex items-center justify-between gap-3">
        ...
        @if ($player)
            {{-- Wallet balance at a glance — tap to top up. --}}
            <a href="{{ route('wallet') }}" wire:navigate
               class="flex shrink-0 items-center gap-2 rounded-xl border border-gold-500/25 bg-gold-500/10 px-3 py-2 focus-ring"
               aria-label="{{ __('Balance') }}: {{ number_format((float) $player->balance, 2) }} {{ config('lottery.currency', 'ETB') }}">
                <svg class="h-4 w-4 text-gold-400" fill="none" viewBox="0 0 24 24" stroke-width="1.7" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 12a2.25 2.25 0 0 0-2.25-2.25H15a3 3 0 1 1-6 0H5.25A2.25 2.25 0 0 0 3 12m18 0v6a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 18v-6m18 0V9M3 12V9m18 0a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 9m18 0V6a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 6v3" />
                </svg>
                <span class="text-sm font-bold tabular-nums text-gold-300">{{ number_format((float) $player->balance, 2) }}</span>
                <span class="text-[10px] font-medium uppercase text-slate-400">{{ config('lottery.currency', 'ETB') }}</span>
            </a>
        @endif
    </header>
    ...
    @else
        ...
        <section class="card mt-4 p-4"
            ...
            <div class="mb-3 flex items-center justify-between">
                ...
                @if ($round->allow_half_tickets)
                    <div class="flex rounded-lg bg-white/5 p-0.5 text-xs">
                        ...
                        <button type="button" @click="mode='half'" :class="mode==='half' ? 'bg-sky-400 text-ink-900' : 'text-slate-300'" class="rounded-md px-3 py-1 font-semibold transition">½ {{ __('Half') }}</button>
                    </div>
                @endif
            </div>
            ...
            <div class="mb-2 flex flex-wrap gap-3 text-[10px] text-slate-400">
                ...
                <span class="flex items-center gap-1"><span class="h-2 w-2 rounded-full bg-sky-400" aria-hidden="true"></span>{{ __('half open') }}</span>
        ...
        @if ($player)
            <section class="card mt-4 p-4 text-center">
                <h3 class="font-semibold">👥 {{ __('Invite your friends') }}</h3>
                <p class="mt-1 text-xs text-slate-400">
                    {{ __(':count invited', ['count' => $player->referral_count]) }} ·
                    {{ __('climb the inviters leaderboard') }} 🏆
                </p>
